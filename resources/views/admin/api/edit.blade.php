            ...
            <div class="panel-body">
                <form class="form-horizontal" id="form" action="{{ route('api.update', ['id' => $data->id]) }}" method="post">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label for="sort" class="col-sm-2 control-label">API别名</label>
                            <div class="col-sm-7">
                                <input type="text" class="form-control" id="api_title" name="api_title" placeholder="" value="{{ $data->api_title }}" />
                            </div>
                        </div>		
                        <div class="form-group">
                            <label for="sort" class="col-sm-2 control-label">开始游戏</label>
                            <div class="col-sm-7">
                                <input type="text" class="form-control" id="start_game_link" name="start_game_link" placeholder="输入开始游戏链接" value="{{ $data->start_game_link }}" />
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="sort" class="col-sm-2 control-label">新窗口打开</label>
                            <div class="col-sm-7">
                                <select name="is_target" class="form-control">
									<option value="0" @if($data->is_target == 0) selected @endif>否</option>
									<option value="1" @if($data->is_target == 1) selected @endif>是</option>
								</select>
                            </div>
                        </div>						
                        <div class="form-group">
