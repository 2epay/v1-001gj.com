...
function NationalDayA(html, IsShow ,sUrl) {
    ...
    if (IsShow) {
        ...
        _box.on('click', '#NationalDayA-con', function(event) {
			if( top.uid  && top.uid != "guest"){ 
				window.open( "/app/member/account/phone_passwd.php?uid="+top.uid); 
			}else{
				window.open( "http://lebophone.com/"); 
			}
		});
		
		_box.hover(
			function(){
				_title.css({'backgroundPosition': '0 0px'})
			},function(){
				_title.css({'backgroundPosition': '0 -30px'})
			}
		);
...
function FestivalDay(html, IsShow ,sUrl) {
    var checkIE6 = document.createElement("b"), isIE6;
    checkIE6.innerHTML = "<!--[if IE 6]><br><![endif]-->";
    isIE6 = (checkIE6.getElementsByTagName("br").length === 1);
    if (IsShow) {
        ...
        var _win = $(window),
            _box = $('#FestivalDay'),
			_title = $("#FestivalDay-title"),
			_con = $("#FestivalDay-con"),
            //_boxH = _box.height(),
            timeoutID;
        ...
        _box.on('click', '#FestivalDay-title', function(event) {
            _win.off('scroll resize', reset);
            window.clearTimeout(timeoutID);
            _box.fadeOut(500, function() { $(this).hide();});
		});
