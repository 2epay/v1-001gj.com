...
//	$("#submit").html('提交中...请稍等');
	canWin = $("#canWin").html();
	if (confirm("可赢金额为:"+canWin+',确定要投注吗?')){
...
function selectMatch( match_id,stype,type,btype){
	...
	if( type =='zhgg'){
		...
		orderFrame.location.replace("/hg_sports/order/zhgg/"+match_id+"?stype="+stype+"&type="+type+"&btype="+btype);
	}else{
		//$("#orderFrame").attr("src","/order/order/"+match_id+"?stype="+stype+"&type="+type+"&btype="+btype);
		orderFrame.location.replace("/hg_sports/order/order/"+match_id+"?stype="+stype+"&type="+type+"&btype="+btype);
	}
}
...
function clickRate( match_id,stype,type,btype){
	parent.parent.orderFrame.selectMatch( match_id,stype,type,btype);
}
...
function cancelOrder(){
	location.href="../../hg_sports/order/index.htm"/*tpa=https://www.237714.com/hg_sports/order/index*/;
}
