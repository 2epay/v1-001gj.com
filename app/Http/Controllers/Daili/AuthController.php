...
class AuthController extends DailiBaseController
{
   // use CurlRequest;
    ...
    public function postApply(Request $request)
    {
		...
		{
			return responseWrong('联系电话必须填写');
		}

		if(!$request->get('msn_qq'))
		{
			return responseWrong('联系QQ必须填写');
		}

		if(!$request->get('about'))
		{
			return responseWrong('申请理由必须填写');
		}
		...
		MemberDailiApply::create([
			...
			'msn_qq' => $request->get('msn_qq'),
			'about' => $request->get('about')
		]);

        return responseSuccess('申请成功', route('member.apply_agent'));
    }
