...
class IndexController extends WebBaseController
{
    ...
    public function eGame(Request $request)
    {
        ...
        $mod = $mod->where('api_id', $api_mod->id)->where('game_type', 3);
		...
		$mod = $mod->whereIn('client_type', [2,3]);
        $game_list = $mod->where('on_line', 0)->orderBy('sort', 'asc')->get();
		
        return view('wap.eGame', compact('game_list','gameName', 'api_name', 'hall', 'line'));
    }
    ...
    public function qq_pay1()
    {
        return view('wap.qq_pay1');
    }	
