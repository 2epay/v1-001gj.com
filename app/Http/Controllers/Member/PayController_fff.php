...
class PayController extends WebBaseController
{
    ...
    public function pay(Request $request)
    {
        ...
        $data['versionId'] = $config_mod->third_version;
		
		
        $data['merId'] = $config_mod->third_userid;
        $userkey = $config_mod->third_userkey;

        $data['asynNotifyUrl'] = route('pay.notify');
        $data['synNotifyUrl'] = route('pay.return');
        $data['prdDisUrl'] = route('pay.return');

        $tradeNo = getBillNo();
        $amount = $request->get('amount')?sprintf("%.2f", $request->get('amount')):0.01;
        $bankId = $request->get('bankcode')?:'ICBC';
		$amount=$amount*100;
        $data['prdOrdNo'] = $tradeNo;
		...
		$data['prdName']='充值';
		$data['prdShortName']='充值';
		$data['prdDesc']='充值';
		$data['pnum']='1';
		
		$sign=$this->sign($data,$userkey);
