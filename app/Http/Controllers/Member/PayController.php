<?php

namespace App\Http\Controllers\Member;

use App\Http\Controllers\Web\WebBaseController;
use App\Models\Member;
use App\Models\PayRecord;
use App\Models\Recharge;
use App\Models\SystemConfig;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Services\PayService;
use Illuminate\Support\Facades\Log;

class PayController extends WebBaseController
{
    public function pay(Request $request)
    {
        $config_mod = SystemConfig::find(1);
        $data['version'] = $config_mod->third_version;
        $data['customerid'] = $config_mod->third_userid;
        $userkey = $config_mod->third_userkey;

        $data['notifyurl'] = route('pay.notify');
        $data['returnurl'] = route('pay.return');


        $tradeNo = getBillNo();
        $amount = $request->get('amount')?sprintf("%.2f", $request->get('amount')):0.01;
        $bankId = $request->get('bankcode')?:'ICBC';
		if ($amount < 10)
            exit('最低充值10元');
		
        $data['sdorderno'] = $tradeNo;
        $data['total_fee'] = $amount;

        $data['paytype'] = $request->get('paytype')?:'bank'; //默认网银直连
        $data['bankcode'] = $bankId;

        $data['get_code'] = $request->get('get_code')?:0;
        $data['remark'] = $request->get('remark');
        $sign=md5('version='.$data['version'].'&customerid='.$data['customerid'].'&total_fee='.$data['total_fee'].'&sdorderno='.$data['sdorderno'].'&notifyurl='.$data['notifyurl'].'&returnurl='.$data['returnurl'].'&'.$userkey);

        $data['sign'] = $sign;
        $clientIp = $request->getClientIp();

        $member = $this->getMember();
        //产生支付记录
        PayRecord::create([
            'member_id' => $member->id,
            'order_no' => $tradeNo,
            'money' => $amount,
            'pay_type' => 1,
            'bankId' => $bankId,
            'clientIp' => $clientIp,
            'before_request_result' => json_encode($data)
        ]);
        //充值记录
        Recharge::create([
            'bill_no' => $tradeNo,
            'member_id' => $member->id,
            'name' => $member->name,
            'money' => $amount,
            'payment_type' => 4,
            'account' => '第三方支付',
            'status' => 1,
            'hk_at' => date('Y-m-d H:i:s')
        ]);

        // 生成表单数据
        //echo $this->buildForm($data,$config_mod->third_pay_url);


		if($data['paytype']=='weixin'){
			$payway='微信';
			$biz_code="1001";
		}elseif($data['paytype']=='alipay'){
			$payway='支付宝';
			$biz_code="1002";
		}elseif($data['paytype']=='weixinwap'){
			$payway='微信';
			$biz_code="1001";
		}elseif($data['paytype']=='alipaywap'){
			$payway='支付宝';
			$biz_code="1002";
		}else{
			$pay_url = $config_mod->third_pay_urlw.'/yunwei_sys/services/order/onlineBank'; // 请求数据地址
			$bank_code=$data['paytype'];
			$userkey = $config_mod->third_userkeyw;
			$key_ret = $userkey; //商户KEY
			$data                = array();
			$data['card_type']=0;
			$data['merchant_id'] = $config_mod->third_useridw; //源付商户号
			$data['bank_code']    = $bank_code; //业务类型
			$data['order_id']    = $tradeNo; //商户流水号
			$data['order_amt']   = $amount*100; //订单金额
			$data['bg_url']      = route('pay.notifyw');   //支付结果回调url
			$data['return_url']  = route('pay.return');   //支付完成页面回调url
			$data_request = $this->getParamString($data,$key_ret);
			//print_r($data_request);exit;
			echo $this->buildForm($data_request,$pay_url);exit;
		}
		//$biz_code=1001;
		//print_r($data);exit;
		
		$pay_url = $config_mod->third_pay_url.'/yunwei_sys/services/mainScan/pay'; // 请求数据地址
		$key_ret = $userkey; //商户KEY
		$data                = array();
		$data['merchant_id'] = $config_mod->third_userid; //源付商户号
		$data['biz_code']    = $biz_code; //业务类型
		$data['order_id']    = $tradeNo; //商户流水号
		$data['order_amt']   = $amount*100; //订单金额
		$data['bg_url']      = route('pay.notify');   //支付结果回调url
		$data['return_url']  = route('pay.return');   //支付完成页面回调url
		
		$data_request = $this->getParamString($data,$key_ret);
		if($biz_code==1003){
			$pay_url = $config_mod->third_pay_url.'/yunwei_sys/services/wap/pay'; // 请求数据地址
			echo $this->buildForm($data_request,$pay_url);exit;
		}elseif($biz_code==1004){
			$pay_url = $config_mod->third_pay_url.'/yunwei_sys/services/wap/pay'; // 请求数据地址
			echo $this->buildForm($data_request,$pay_url);exit;
		}
		$return_data = $this->paycurl($pay_url, $data_request);
		//echo '请求参数';print_r($data_request);echo '<br>';
		//echo '返回参数';print_r($return_data);
		$return_data = json_decode($return_data, true);

		if ($return_data['rsp_code'] != 00) {
			echo ("获取二维码失败 代码 {$return_data['rsp_code']}：" . $return_data['rsp_msg']);
			exit;
		}
		else {
			
			include($_SERVER['DOCUMENT_ROOT'].'/ewm.php');
			//qrcode($return_data['pay_url']);
		}		
		
		
		
    }
	
	function paycurl($url, $data = NULL) {
		$data = json_encode($data);

		//echo ("请求地址：" . $url);
		//echo '<br>';
		//echo ("请求JSON：" . $data);
		//echo '<br>';

		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, $url);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false);

		curl_setopt($curl, CURLOPT_POST, 1);
		curl_setopt($curl, CURLOPT_POSTFIELDS, $data);

		curl_setopt($curl, CURLOPT_HEADER, 0);
		curl_setopt($curl, CURLOPT_HTTPHEADER, array(
			'Content-Type: application/json; charset=utf-8',
			'Content-Length:' . strlen($data))
		);

		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		$res     = curl_exec($curl);
		$errorno = curl_errno($curl);
		curl_close($curl);
		if ($errorno) {
			return array('errorno' => false, 'errmsg' => $errorno);
		}

		return $res;
	}	
	function getParamString($params,$key_ret) {


		//空值不传递
		foreach ($params as $key => $value) {
			if (empty($value)) {
				//unset($params[$key]);
			}
		}
		//计算签名（sign、rsp_code、rsp_msg 不参加）
		$paramsSign = array();
		foreach ($params as $key => $value) {
			if ($key != 'sign' && $key != 'rsp_code' && $key != 'rsp_msg') {
				$paramsSign[$key] = $value;
			}
		}

		//按照key排序
		ksort($paramsSign);

		$r = http_build_query($paramsSign) . '&' . $key_ret;

		//echo ("MD5前：" . urldecode($r));
		//echo '<br>';
		//加密获取sign
		$sign = strtoupper(md5(urldecode($r))); //对该字符串进行 MD5 计算，得到签名，并转换大写

		//echo ("MD5后：" . $sign);
		//echo '<br>';
		//设置请求参数
		$params['sign'] = $sign;

		return $params;
	}
	
	public function pay_scan(Request $request)
    {
        $config = config('pay');
        $data['service'] = $config['APINAME_SCANPAY'];
        $data['version'] = $config['API_VERSION'];
        $data['merId'] = $config['MERCHANT_ID'];

        $tradeNo = getBillNo();
        $amount = $request->get('amount')?sprintf("%.2f", $request->get('amount')):0.01;
        $typeId = $request->get('typeId')?:1;
        $clientIp = $request->getClientIp();

        //
        if ($typeId == 1)
        {
            if ($amount> 2000 || $amount < 100)
                return respF('支付宝扫码充值范围100-2000元');
        } elseif ($typeId ==2) {
            if ($amount > 3000 || $amount < 100)
                return respF('微信扫码充值范围100-3000元');
        }


        $data['tradeNo'] = $tradeNo;
        $data['tradeDate'] = date('Ymd');
        $data['amount'] = $amount;
        $data['notifyUrl'] = route('pay.notify');
        $data['extra'] = '';
        $data['summary'] = '在线充值';
        $data['expireTime'] = 60;
        $data['clientIp'] = $clientIp;
        //类型 微信 支付宝
        $data['typeId'] = $typeId;

        $data = $this->transfer($data);
        // 初始化
        $pPay = new PayService($config['KEY'],$config['GATEWAY_URL']);
        // 准备待签名数据
        $str_to_sign = $pPay->prepareSign($data);
        // 数据签名
        $sign = $pPay->sign($str_to_sign);
        // 准备请求数据
        $to_requset = $pPay->prepareRequest($str_to_sign, $sign);

        $member = $this->getMember();
        //产生支付记录
        PayRecord::create([
            'member_id' => $member->id,
            'order_no' => $tradeNo,
            'money' => $amount,
            'pay_type' => 2,
            'typeId' => $typeId,
            'clientIp' => $clientIp,
            'before_request_result' => $to_requset
        ]);
        //充值记录
        Recharge::create([
            'bill_no' => $tradeNo,
            'member_id' => $member->id,
            'name' => $member->name,
            'money' => $amount,
            'payment_type' => 4,
            'account' => '第三方支付',
            'status' => 1,
            'hk_at' => date('Y-m-d H:i:s')
        ]);

        //请求数据
        $resultData = $pPay->request($to_requset);
        // 准备验签数据
        $to_verify = $pPay->prepareVerify($resultData);
        // 签名验证
        $resultVerify = $pPay->verify($to_verify[0], $to_verify[1]);
        if ($resultVerify) {
            // 响应吗
            preg_match('{<code>(.*?)</code>}', $resultData, $match);
            $respCode = $match[1];
            // 响应信息
            preg_match('{<desc>(.*?)</desc>}', $resultData, $match);
            $respDesc = $match[1];

            if ($respCode == '00')
            {
                preg_match('{<qrCode>(.*?)</qrCode>}', $resultData, $match);
                $respqrCode= $match[1];

            } else {
                echo $respDesc;exit;
            }


            $base64 =base64_decode($respqrCode);

            $value="http://www.useryx.com";
            $errorCorrectionLevel = "L"; // 纠错级别：L、M、Q、H
            $matrixPointSize = "4"; // 点的大小：1到10
            //echo  $base64;

            //EWM();
        } else {
            echo "验证签名失败";
            return false;
        }

        if ($request->has('is_m') && $request->get('is_m') == 1)
        {
            return view('wap.pay_scan', compact('base64', 'value', 'errorCorrectionLevel', 'matrixPointSize', 'typeId'));
        }

        return view('member.pay_scan', compact('base64', 'value', 'errorCorrectionLevel', 'matrixPointSize', 'typeId'));
    }

    public function success()
    {
        $r = route('member.customer_report').'?type=0';
        $agent = $_SERVER['HTTP_USER_AGENT'];
        if(strpos($agent,"comFront") || strpos($agent,"iPhone") || strpos($agent,"MIDP-2.0") || strpos($agent,"Opera Mini") || strpos($agent,"UCWEB") || strpos($agent,"Android") || strpos($agent,"Windows CE") || strpos($agent,"SymbianOS"))
        {
            $r = route('wap.recharge_record');
        }
        return view('member.pay_success', compact('r'));
    }

    public function notify(Request $request)
    {
        $res = $request->all();
		
		
		file_put_contents("./111.txt", json_encode($_GET).json_encode($_POST).json_encode($res)."\r\n",FILE_APPEND);
		/*
		$json=json_decode('{"merchant_id":"142651705539039232","order_amt":"1","order_id":"20180201163150J5Hj1","state":"0","up_order_id":"784946399971155968992282","sign":"5DABB0A85DD9B93C1CA8CC89800D7E3B"}',true);
		foreach($json as $k=>$v){
			$res[$k]=$v;
		}
		*/
        Log::info($res);
        // 请求数据赋值
        $data = [];
		//print_r($json);exit;
        // 订单状态，0-未支付，1-支付成功，2-失败，4-部分退款，5-退款，9-退款处理中
		//交易实际状态，0：成功，1：失败，2：处理中
        $data['order_amt'] = $res["order_amt"];
        $data['up_order_id'] = $res["up_order_id"];
        $data['state'] = $res['state'];
        $data['order_id'] = $res['order_id'];
        $data['merchant_id'] = $res['merchant_id'];
        
		
        $config_mod = SystemConfig::find(1);
		$data_request = $this->getParamString($data,$config_mod->third_userkey);
		$data['sign'] = $res['sign'];
		if($data['sign']!=$data_request['sign']){
			exit('fail');
		}
		
		//exit;
        //处理订单
        $mod = PayRecord::where('order_no', $data["order_id"])->first();
        if ($mod)
        {
            $member = Member::find($mod->member_id);
            $recharge = Recharge::where('bill_no', $data["order_id"])->first();
            $mod->update([
                'status' => $data["state"],
                'after_request_result' => json_encode($data)
            ]);


            //如果处理成功
            if ($recharge)
            {
                if ($data['state'] == 0 && $recharge->status != 2)
                {
                    //中心账户
                    $member->increment('money', $data["order_amt"]/100);
                    //支付记录
                    $recharge->update([
                        'status' =>2,
                        'confirm_at' => date('Y-m-d H:i:s'),
                    ]);
                } 
				
				/*
				else {
                    $recharge->update([
                        'status' =>3,
                    ]);
                }
				*/
            }
        }

		exit('ok');
    }
    public function notifyw(Request $request)
    {
        $res = $request->all();
		
		
		file_put_contents("./222.txt", json_encode($_GET).json_encode($_POST).json_encode($res)."\r\n",FILE_APPEND);
		/*
		$json=json_decode('{"merchant_id":"142651705539039232","order_amt":"1","order_id":"20180201163150J5Hj1","state":"0","up_order_id":"784946399971155968992282","sign":"5DABB0A85DD9B93C1CA8CC89800D7E3B"}',true);
		foreach($json as $k=>$v){
			$res[$k]=$v;
		}
		*/
        Log::info($res);
        // 请求数据赋值
        $data = [];
		//print_r($json);exit;
        // 订单状态，0-未支付，1-支付成功，2-失败，4-部分退款，5-退款，9-退款处理中
		//交易实际状态，0：成功，1：失败，2：处理中
        $data['order_amt'] = $res["order_amt"];
        $data['up_order_id'] = $res["up_order_id"];
        $data['state'] = $res['state'];
        $data['order_id'] = $res['order_id'];
        $data['merchant_id'] = $res['merchant_id'];
        
		
        $config_mod = SystemConfig::find(1);
		$data_request = $this->getParamString($data,$config_mod->third_userkeyw);
		$data['sign'] = $res['sign'];
		if($data['sign']!=$data_request['sign']){
			exit('fail');
		}
		
		//exit;
        //处理订单
        $mod = PayRecord::where('order_no', $data["order_id"])->first();
        if ($mod)
        {
            $member = Member::find($mod->member_id);
            $recharge = Recharge::where('bill_no', $data["order_id"])->first();
            $mod->update([
                'status' => $data["state"],
                'after_request_result' => json_encode($data)
            ]);


            //如果处理成功
            if ($recharge)
            {
                if ($data['state'] == 0 && $recharge->status != 2)
                {
                    //中心账户
                    $member->increment('money', $data["order_amt"]/100);
                    //支付记录
                    $recharge->update([
                        'status' =>2,
                        'confirm_at' => date('Y-m-d H:i:s'),
                    ]);
                } 
				
				/*
				else {
                    $recharge->update([
                        'status' =>3,
                    ]);
                }
				*/
            }
        }

		exit('ok');
    }
    public function notifym(Request $request)
    {
        $res = $request->all();
		
		
		file_put_contents("./333.txt", json_encode($_GET).json_encode($_POST).json_encode($res)."\r\n",FILE_APPEND);
		/*
		$json=json_decode('{"merchant_id":"142651705539039232","order_amt":"1","order_id":"20180201163150J5Hj1","state":"0","up_order_id":"784946399971155968992282","sign":"5DABB0A85DD9B93C1CA8CC89800D7E3B"}',true);
		foreach($json as $k=>$v){
			$res[$k]=$v;
		}
		*/
        Log::info($res);
        // 请求数据赋值
        $data = [];
		//print_r($json);exit;
        // 订单状态，0-未支付，1-支付成功，2-失败，4-部分退款，5-退款，9-退款处理中
		//交易实际状态，0：成功，1：失败，2：处理中
        $data['order_amt'] = $res["order_amt"];
        $data['up_order_id'] = $res["up_order_id"];
        $data['state'] = $res['state'];
        $data['order_id'] = $res['order_id'];
        $data['merchant_id'] = $res['merchant_id'];
        
		
        $config_mod = SystemConfig::find(1);
		$data_request = $this->getParamString($data,$config_mod->third_userkeym);
		$data['sign'] = $res['sign'];
		if($data['sign']!=$data_request['sign']){
			exit('fail');
		}
		
		//exit;
        //处理订单
        $mod = PayRecord::where('order_no', $data["order_id"])->first();
        if ($mod)
        {
            $member = Member::find($mod->member_id);
            $recharge = Recharge::where('bill_no', $data["order_id"])->first();
            $mod->update([
                'status' => $data["state"],
                'after_request_result' => json_encode($data)
            ]);


            //如果处理成功
            if ($recharge)
            {
                if ($data['state'] == 0 && $recharge->status != 2)
                {
                    //中心账户
                    $member->increment('money', $data["order_amt"]/100);
                    //支付记录
                    $recharge->update([
                        'status' =>2,
                        'confirm_at' => date('Y-m-d H:i:s'),
                    ]);
                } 
				
				/*
				else {
                    $recharge->update([
                        'status' =>3,
                    ]);
                }
				*/
            }
        }

		exit('ok');
    }

    public function pay_return(Request $request)
    {
        return redirect()->to(route('pay.success'));
    }
    
    public function houtai(Request $request)
    {
        $out_trade_no=$request->get('out_trade_no');
		$mod = Recharge::where('bill_no', $out_trade_no)->first();
        if ($mod)
        {
			echo $mod['status'];
		}
		
	}
    protected  function buildForm($data, $gateway) {
        $sHtml = "<form id='paysubmit' name='bankPaySubmit' action='".$gateway."' method='post'>";
        while (list ($key, $val) = each ($data)) {
            $sHtml.= "<input type='hidden' name='".$key."' value='".$val."'/>";
        }
        $sHtml.= "</form>";
        $sHtml.= "<script>document.forms['bankPaySubmit'].submit();</script>";

        return $sHtml;
    }
}
