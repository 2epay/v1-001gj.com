...
class GameListController extends AdminBaseController
{
    public function index(Request $request)
    {
        ...
        $game_line = $name = $game_type = $api_id = $client_type = '';
        ...
        if ($request->has('game_line'))
        {
            $game_line = $request->get('game_line');
            $mod = $mod->where('game_line', $game_line);
        }		


        $data = $mod->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));

        return view('admin.game_list.index', compact('data', 'name','game_type', 'api_id', 'client_type', 'game_line'));
    }
