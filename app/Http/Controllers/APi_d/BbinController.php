...
class BbinController extends WebBaseController
{
    ...
    public function game_record_d_game()
    {
        return view('api_d.getRecord_bbin_game');
    }

    public function game_record_d_live()
    {
        return view('api_d.getRecord_bbin_live');
    }

    public function game_record_d_lottery()
    {
        return view('api_d.getRecord_bbin_lottery');
    }

    public function game_record_d_ball()
    {
        return view('api_d.getRecord_bbin_ball');
    }
        ...
        foreach ($list as $item)
        {
            ...
            if ($res['Code'] ==0)
            {
                ...
                if (count($data) > 0)
                {
                    foreach ($data as $value)
                    {
                        $m = GameRecord::where('api_type', $this->api->id)->where('billNo', $value['WagersID'])->first();
                        if (!$m)
                        {
                            $l = strlen($this->api->prefix);
                            $PlayerName = $value["UserName"];
